	...
	if (steps > 0)
	{
		for (std::size_t l = 0; l < this->model->getOperationalDof(); ++l)
		{
			for (std::size_t i = 0; i < steps + 1; ++i)
			{
				...
				auto t = this->model->forwardPosition(l);
...
Viewer::drawConfigurationPath(const rl::plan::VectorList& path)
{
	...
	for (std::size_t l = 0; l < this->model->getOperationalDof(); ++l)
	{
		...
		for (; i != path.end() && j != path.end(); i = j, ++j)
		{
			auto dis = this->model->distance(*i, *j);
			while(dis < this->delta)
			{
				++j;
				if(j == path.end())
				{
				...
				dis = this->model->distance(*i, *j);
			}
			rl::math::Real steps = std::ceil(dis / this->delta);
...
void
Viewer::saveImage(const QString& filename,bool withAlpha)
{
...
void Viewer::saveScene(const QString& filename)
{
	SoOutput output;
...
void Viewer::showMessage(const std::string& message)
{
